...
def compute_geodesic_distance_from_two_matrices(m1, m2):
    ...
    cos = torch.min(cos, torch.autograd.Variable(torch.ones(batch)).cuda()).cuda()
    cos = torch.max(cos, torch.autograd.Variable(torch.ones(batch)).cuda() * -1).cuda()

    theta = torch.acos(cos).cuda()
...
def compute_quaternions_from_rotation_matrices(matrices):
    ...
    w = torch.sqrt(
        torch.max(1.0 + matrices[:, 0, 0] + matrices[:, 1, 1] + matrices[:, 2, 2], torch.zeros(1).cuda())) / 2.0
...
def compute_nearest_10d(x, qg, prev_eigenval=None):
    ...
    if prev_eigenval is None:
        ...
        eigenval = (torch.bmm(torch.bmm(qgs.transpose(-1, -2), KKT_M), Aqs) + torch.bmm(
            torch.bmm(Aqs.transpose(-1, -2), KKT_M), qgs)) / (
                               2 * torch.bmm(torch.bmm(qgs.transpose(-1, -2), KKT_M), qgs))
    else:
